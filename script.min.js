...
function dashboard() {
  ...
  otherUsersDiv.src=`explorer.html?gid=${uid}&gname=1&emb=emb`;
  let x = {
	id:uid,
	name:un,
	title:1,
	...
	firebase.database().ref().child('mails').child(uid).child('mails').get().then(snapshot=>{
		if(snapshot.exists()){
			...
			for (i in myStaffData) {
				...
				<button style="${xbtn + "background-color:gray;"}" onclick="addToExplore('${i}')">add to explore</button>
				<button style="${xbtn}" onclick="deletePost('${i}')">delete</button>
				`;
...
function deletePost(name){
	if(confirm("Are you sure you want to delete this post?")){
		firebase.database().ref().child('mails').child(uid).child('mails').child(name).set({});
		reloadMyStaff();
	}
}
...
function addToExplore(name){
	firebase.database().ref().child('posts').child('myanmar').child(name).set({
		link:`https://explore.com?gid=${uid}&gname=${name}`,
		likes:{
			2:"YeaeThawe",
		},
		name:un,
	});
	alert('Your post added to Explore successfully.');
}
